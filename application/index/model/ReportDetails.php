<?php
namespace app\index\model;

//导入模型类
use think\Model;
use app\index\model\Users;

class ReportDetails extends Model {
	
	// 设置当前模型对应的完整数据表名称
    protected $table = 'view_ReportDetails';

	//在子类重写父类的初始化方法initialize()
	protected function initialize(){
		
	  //继承父类中的initialize()
		parent::initialize();
		
	   //初始化数据表字段信息	
	   $this->field = $this->db()->getTableInfo('', 'fields');  
	
	   //初始化数据表字段类型
	   $this->type = $this->db()->getTableInfo('', 'type'); 
	
	   //初始化数据表主键
	   $this->pk = $this->db()->getTableInfo('', 'pk');     
		
		
		}

	public function getReportList($result_type = 0){
		$report = $this->where('result_type', 'in', $result_type)
		->order('report_date','asc')
		->paginate(20);
		return $report;
	}
	
	public function getReportDetails($reportid){
		$report = $this->where('reportid', $reportid)
		->find();
		return $report;
	}

	public function users()
    {
        return $this->belongsTo('Users','userid');
    }
}